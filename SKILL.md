...
5. 업무 문제는 한 문장에 몰아 묻지 않고, 반복 업무 -> 가장 막히는 사람 -> 막히는 순간 순서로 2-3번에 나눠 확인합니다. 사용자가 이미 세 요소를 모두 말했으면 후속 질문은 생략합니다.
6. 자료가 있으면 개인정보 제거 샘플 기준을 확인합니다. `오늘 써볼 실제 자료가 있나요?`를 물을 때는 오렌지필터가 개인정보와 민감정보를 익명화해 워크숍용 샘플을 만들 때 쓰는 Chrome 확장 프로그램이라는 짧은 소개와 설치 링크(https://chromewebstore.google.com/detail/%EC%98%A4%EB%A0%8C%EC%A7%80%ED%95%84%ED%84%B0orange-filter-ai-%ED%94%84%EB%9D%BC%EC%9D%B4/gnkcbdgbajeboglkplllicdghlcghjai)를 기본으로 함께 제공합니다. 자료가 없으면 가상 샘플을 만들어도 되는지 확인합니다.
7. 문제를 들은 뒤 자동화 도구, 웹앱, 온보딩 페이지, admin 화면, 대시보드, 문서/메시지 초안 등 가능한 결과물 후보를 2-4개 제안하고 하나를 고릅니다.
8. 범위는 3시간 안에 보여줄 수 있는 완결형 업무 흐름 1개로 좁힙니다. `references/mvp-scope.md` 기준으로 해결할 좁은 문제, 포함할 업무 단계, 오늘 할 것/하지 않을 것, AI 역할, 사람 검토 지점, 성공/실패 기준, 3시간 안/밖 판단을 분리합니다. 성공/실패 기준을 물을 때는 현재 프로젝트에서 이미 확인한 입력 자료, 결과물 형태, 사람 검토 지점, 주요 위험을 바탕으로 예시를 만듭니다. 일반적인 도메인 예시는 해당 프로젝트가 실제로 그 업무일 때만 사용합니다.
9. 문제정의가 끝나면 `PLAN.md`와 `workshop.json` 초안을 생성합니다. 답변을 JSON으로 정규화했다면 `scripts/write_workshop_outputs.py --phase plan`을 우선 사용합니다. `PLAN.md` 작성이 끝나면 파일을 다시 읽어 채팅창에 `PLAN.md` 내용을 보여준 뒤, 스킬이 직접 구현으로 넘어가지 않고 사용자에게 이 계획을 기준으로 구현을 진행하라고 넘깁니다. 이때 `PLAN.md`에는 새 세션에서 구현하더라도 구현자가 `MEMORY.md`를 append-only로 갱신해야 한다는 지침을 남깁니다.
10. `PLAN.md`를 보여준 직후에는 다음 행동을 명확히 안내합니다. 지금은 사용자가 구현을 시작하면 되고, 구현이 완료되면 같은 폴더에서 다시 이 스킬에 발표자료를 만들어달라고 요청하면 된다고 말합니다.
11. 사용자가 구현을 마친 뒤 다시 정리를 요청하면 `MEMORY.md` 파일 끝에 새 항목을 추가하고 `workshop.json`을 갱신합니다. 이전 항목은 덮어쓰지 않습니다. 형식은 `references/memory-log.md`를 따릅니다.
12. 완료 시 `references/output-consistency.md`, `references/intent-check.md`, `references/workflow-analysis.md`를 읽고 구현 결과와 기록을 분석해 `WORKFLOW_ANALYSIS.md`, `CASE_STUDY.md`, 최종 `workshop.json`이 제출/아카이빙 기준을 만족하는지 확인합니다.
13. 발표자료 제작 전 `references/presentation-handoff.md`를 기준으로 최종 입력 폴더를 점검합니다. 최종 기준 데이터는 `workshop.json`이며, `input.json`이 남아 있으면 최종 데이터와 충돌하지 않는지 확인합니다. 공개 가능한 결과물 캡처가 있으면 `presentation-assets/result_screenshot.png`로 준비합니다.
14. 사용자가 발표자료 제작을 요청하면 `references/presentation-build.md`를 따릅니다. 먼저 참가자 폴더에 `WORKFLOW_ANALYSIS.md`와 `CASE_STUDY.md`가 있는지 확인하고, 둘 중 하나라도 없으면 `scripts/write_workshop_outputs.py --phase final`로 두 파일을 먼저 만든 뒤 발표자료 제작을 시작합니다. 그 다음 Google Slides 업로드 동의를 받고, 사회혁신가와 개발자 한줄소개를 한 명씩 받은 뒤, 내장 스크립트 `scripts/prepare-presentation-content.mjs`와 `scripts/build-google-slides-requests.mjs`를 사용해 Google Slides 사본을 채웁니다. 생성한 사본은 반드시 링크가 있는 모든 사용자가 뷰어 권한으로 볼 수 있도록 공유 설정합니다.
...
- 자유 서술이 필요한 질문은 `request_user_input`이 활성화되어 있으면 도구로 묻고, 이때 선택지는 답변 방식을 고르는 2-3개 옵션으로 만듭니다. 도구가 자유 입력 옵션을 자동으로 붙이므로 `직접 입력` 옵션을 별도로 만들지 않습니다.
...
- 업무 문제 질문은 기본적으로 2-3개로 나눕니다. 먼저 반복 업무 1개를 묻고, 답변에 빠졌을 때만 어려움을 겪는 사람과 문제가 발생하는 순간을 각각 후속 질문으로 묻습니다.
- 실제 자료가 없으면 워크숍을 멈추지 않습니다. 비식별 가상 샘플을 만들고 `workshop.json`과 산출물에 가상 샘플임을 표시합니다.
- `오늘 써볼 실제 자료가 있나요?`를 물을 때는 항상 오렌지필터가 개인정보와 민감정보를 익명화해 워크숍용 샘플을 만들 때 쓰는 Chrome 확장 프로그램이라고 짧게 소개하고 설치 링크를 함께 제공합니다.
...
- `PLAN.md`를 만들 때는 오늘 만들 핵심 기능 1개와 별도로, 핵심 기능이 끝났을 때 이어서 할 작업 후보 2개를 반드시 제안합니다. 각 후보에는 시작 조건, 구현 내용, 데모 산출물, 예상 추가 시간, 검수 기준을 포함합니다.
- 성공/실패 기준을 물을 때는 이 프로젝트의 샘플 입력과 결과물 기준 예시를 함께 제시합니다. 예시는 사용자가 말한 업무 문제, 자료 형태, 선택한 결과물, 사람 검토 지점에서 끌어와 만들고, 일반적인 예시는 주지 않습니다. 최소한 "무엇이 나오면 성공인지"와 "무엇이 남거나 틀리면 실패인지"를 모두 포함해야 합니다.
- 문제정의와 `PLAN.md` 작성이 끝난 직후에는 구현을 시작하지 않습니다. 구현은 사용자가 맡으며, 스킬은 `PLAN.md` 전체 내용을 채팅창에 보여주고 다음 행동을 넘겨주는 데서 멈춥니다. 단, `PLAN.md`에는 이후 새 세션의 구현자가 구현 중 결정, 변경, 막힌 점, 검증 결과를 `MEMORY.md`에 append-only로 기록하라는 지침을 반드시 포함합니다.
