...
8. 범위는 3시간 안에 보여줄 수 있는 완결형 업무 흐름 1개로 좁힙니다. `references/mvp-scope.md` 기준으로 해결할 좁은 문제, 포함할 업무 단계, 오늘 할 것/하지 않을 것, AI 역할, 사람 검토 지점, 성공/실패 기준, 3시간 안/밖 판단을 분리합니다. 성공/실패 기준을 물을 때는 "신청서 1건에서 누락 항목 2개 이상을 찾으면 성공, 개인정보가 출력에 남으면 실패"처럼 샘플 입력 기준 예시를 함께 제시합니다.
...
- 성공/실패 기준을 물을 때는 샘플 입력과 결과물 기준 예시를 함께 제시합니다. 예시는 분야를 그대로 맞추되, 최소한 "무엇이 나오면 성공인지"와 "무엇이 남거나 틀리면 실패인지"를 모두 포함해야 합니다.
