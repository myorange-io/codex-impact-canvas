...
description: 사회문제 해결 해커톤 팀이 준비해 온 현장 문제와 샘플 자료를 3시간 안에 시연 가능한 완결형 AI 에이전트 MVP로 압축하고, 필수 마크다운 산출물 작성을 반응형 질문으로 진행하도록 돕는 스킬입니다. 사용자가 /codex-impact-canvas라고 입력하거나, 현장 업무를 하나의 좁지만 끝까지 해결되는 AI 에이전트 MVP로 수렴하고, PLAN.md와 MEMORY.md를 작성하고, WORKFLOW_ANALYSIS.md로 AI 에이전트 워크플로를 분석하고, 공개 가능한 CASE_STUDY.md를 만들 때 사용합니다.
---

# Codex Impact Canvas

이 스킬은 2인 워크숍 팀이 준비해 온 현장 문제와 샘플 자료를 바탕으로 3시간 안에 보여줄 수 있는 완결형 AI 에이전트 MVP 1개로 범위를 압축하고, 아래 네 개의 필수 마크다운 산출물을 만들도록 안내합니다.
...
3. `references/canvas-flow.md`, `references/problem-archetypes.md`, `references/multiple-choice.md`를 읽고 문제 확인 -> 유형 분기 -> 객관식 수렴 -> MVP 압축 흐름을 진행합니다.
4. 처음 세 질문 안에 문제 유형, 샘플 입력 업무 묶음, 기대 최종 산출물을 확정합니다. 이미 답한 내용은 다시 묻지 않고 빠진 정보만 묻습니다.
5. `references/mvp-scope.md`를 읽고 범위를 좁힙니다. 목표는 단순한 기능 1개가 아니라 좁은 문제를 시작부터 검토 가능한 결과까지 끝내는 완결형 업무 흐름 1개입니다. 핵심 흐름을 먼저 끝낸 뒤 시간이 남으면 이어서 할 작업 후보 2개도 시작 조건, 구현 내용, 데모 산출물, 예상 추가 시간, 검수 기준까지 정리합니다.
...
- 여러 방향이 섞였거나 합의가 느려지면 `references/multiple-choice.md`의 객관식 프롬프팅 또는 객관식 플래닝으로 2-3개 선택지를 제시하고, 추천안을 하나 명시합니다.
- 객관식 선택지는 사용자의 핵심 자동화 기대를 모두 같은 수준으로 충족해야 합니다. 산출물 자동 생성을 원하면 검수표나 입력 보조만 단독 선택지로 두지 않습니다.
- 3시간 MVP가 너무 단순해 보이면 새 시스템을 붙이기보다 근거, 예외 처리, 검토 체크리스트, 전달용 산출물, 샘플 3-5건 묶음처럼 업무 완결성을 높입니다.
...
- `references/multiple-choice.md`: 객관식 프롬프팅과 객관식 플래닝 운영 규칙
- `references/mvp-scope.md`: 3시간 완결형 MVP 안/밖 판단 기준과 깊이 조절 규칙
