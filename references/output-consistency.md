...
- `PLAN.md` 작성 직후: 스킬이 직접 구현하지 않고 사용자가 `PLAN.md` 기준으로 구현
...
문제정의 직후에는 최종 분석 문서를 미리 만들지 않도록 `--phase plan`을 사용합니다. 이 단계에서 스킬은 직접 구현하지 않고 사용자에게 `PLAN.md` 기준 구현을 넘깁니다. 구현 완료 후 분석과 사례 정리를 만들 때는 `--phase final`을 사용합니다.
