...
워크숍 중 사용자에게 질문할 때는 일반 채팅 문장 대신 `request_user_input` 방식으로 묻습니다. 한 번에 하나의 질문만 띄우고, 자유 서술이 필요한 경우에도 `직접 입력` 선택지를 포함한 구조화된 질문으로 진행합니다.
...
2. 기본 정보 확인: `request_user_input`으로 사회혁신가 이름, 개발자 이름, 팀 또는 프로젝트 이름을 하나씩 순차 질문합니다.
...
실제 진행 시 한 번에 묶어 묻지 않고 아래 순서로 각각 `request_user_input`을 호출합니다.
...
아래 질문들은 실제 진행 시 `request_user_input` 호출로 전달합니다. 이미 확인된 내용은 건너뛰고, 빈칸을 채울 때만 한 번에 하나씩 묻습니다.
