...
```text
$codex-impact-canvas를 사용해 우리 해커톤 팀의 문제 정의를 돕고 필수 마크다운 산출물을 작성해줘.
```

스킬은 먼저 사회혁신가 이름과 개발자 이름을 받고, 여섯 개 앵커 질문을 순서대로 진행합니다. 답변이 추상적이거나 구현 범위로 바로 이어지지 않으면 후속 질문을 합니다. 질문 수는 여섯 개로 고정하지 않습니다. 목표는 `PLAN.md`, `WORKFLOW_ANALYSIS.md`, `CASE_STUDY.md`에 바로 기록할 수 있는 수준까지 답변을 구체화하는 것입니다.
...
1. 팀 시작 정보를 받습니다.
2. 반응형 질문 흐름으로 문제와 현재 업무를 파악합니다.
3. 답변을 요약하고, 부족하면 후속 질문으로 보완합니다.
4. 기능 후보가 여러 개면 3시간 MVP 기능 1개만 남깁니다.
5. 필수 산출물 네 개를 같은 구조로 생성합니다.
6. 구현 또는 분석 단계가 끝날 때마다 `MEMORY.md`에 과정을 누적 기록합니다.
7. 공개 가능한 사례 요약인 `CASE_STUDY.md`에서 개인정보와 내부 원문을 제거했는지 확인합니다.
