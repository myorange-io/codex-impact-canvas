...
4. 문제 유형을 고르고, 여러 방향이 섞이면 객관식 프롬프팅으로 후보를 좁힙니다.
5. 샘플 입력 업무 묶음과 검토 가능한 최종 산출물을 고정합니다.
6. 사람이 반복해서 읽고, 옮기고, 판단하고, 다시 쓰는 지점 하나로 병목을 압축합니다.
7. AI가 할 일, 사람이 검토할 일, 실패 시 이어받는 방식을 분리합니다.
8. 좁은 문제를 끝까지 해결하는 3시간 MVP 흐름과 제외 범위, 빠르게 끝났을 때 할 후보 2개를 정합니다. 필요하면 객관식 플래닝으로 2-3개 MVP안을 비교합니다.
...
여러 유형이 섞이면 A/B/C 선택지로 좁힙니다. 각 선택지에는 한 줄 설명을 붙이고, 추천안에는 추천 이유를 붙입니다.
...
MVP 방향이 여러 개라면 사용자의 자동화 기대를 먼저 맞춥니다. 반복 제작물 자동화를 원한다면 `단일 산출물 자동 제작형`, `복수 산출물 자동 제작형`, `제작 + 커뮤니케이션 자동 생성형`처럼 모두 자동 생성형인 선택지로 비교합니다.
...
답변이 추상적이거나 여러 방향이 섞이면 객관식 프롬프팅 또는 객관식 플래닝을 사용합니다.

- 객관식 프롬프팅: 문제 유형, 산출물, 검토 방식, 제외 범위를 고를 때 사용합니다.
- 객관식 플래닝: MVP 후보 2-3개를 비교하고 추천안을 고를 때 사용합니다.
- 선택지는 2-3개를 기본으로 하고, 모두 선택 가능한 대안으로 만듭니다. 명백히 약한 들러리 선택지는 넣지 않습니다.
- 모든 선택지는 사용자의 핵심 자동화 기대를 같은 수준으로 충족해야 합니다.
- 항상 추천안과 추천 이유를 붙입니다.
- 사용자가 A/B/C만 답해도 다음 단계로 진행합니다.
