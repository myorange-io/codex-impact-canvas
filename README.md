...
6. 핵심 기능이 예상보다 빨리 끝났을 때 바로 이어서 할 작업 후보 2개를 구체화합니다.
7. 필수 산출물 네 개를 같은 구조로 생성합니다.
8. 구현 또는 분석 단계가 끝날 때마다 `MEMORY.md`에 과정을 누적 기록합니다.
9. 공개 가능한 사례 요약인 `CASE_STUDY.md`에서 개인정보와 내부 원문을 제거했는지 확인합니다.
...
- 핵심 기능이 예상보다 빨리 끝나면 바로 이어서 할 작업 후보 2가지는 무엇인가요?
- 각 후보는 어떤 조건에서 시작하고, 무엇을 구현하고, 발표장에서 어떤 결과를 추가로 보여주면 되나요?
- 각 후보를 몇 분 안에 끝내고, 성공 여부는 무엇으로 확인할 수 있나요?
