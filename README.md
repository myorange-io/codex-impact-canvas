...
3. 업무 문제 확인: 반복 처리 업무, 불편을 겪는 사람, 문제가 발생하는 순간을 기본 2-3개 질문으로 나눠 확인합니다.
4. 자료 확인: 실제 자료 유무, 개인정보 제거 샘플, 오렌지필터 소개와 설치 링크, 익명화되지 않은 샘플 처리, 자료가 없을 경우 가상 샘플 생성
5. 결과물 선택: 문제를 들은 뒤 자동화 도구, 웹앱, 온보딩 페이지, admin 화면, 대시보드, 문서/메시지 초안 등 가능한 후보 제안
6. 3시간 MVP 좁히기: 해결할 좁은 문제, 포함할 업무 단계, 오늘 할 것/하지 않을 것, AI 역할, 사람 검토 지점, 예시를 포함한 성공/실패 기준, 3시간 안/밖 판단
7. 다음 단계 안내: `PLAN.md`, `workshop.json` 초안 생성 후 `PLAN.md` 내용을 채팅창에 보여주고, 스킬은 직접 구현하지 않고 사용자에게 구현을 넘김
...
기본적으로 2-3번에 나눠 묻습니다. 한 답변에 반복 업무, 막히는 사람, 막히는 순간이 모두 들어 있으면 남은 질문은 생략합니다.

```text
지금 반복해서 처리하느라 가장 불편한 업무 1개는 무엇인가요?
```

답변에 어려움을 겪는 사람이 빠졌을 때:
...
```text
그 사람이 실제로 막히는 순간은 언제인가요? 방금 말한 업무에서 자료가 들어오는 순간부터 결과를 검토하거나 전달하는 순간까지, 이 프로젝트의 실제 장면 하나로 말해 주세요.
```

이미 세 요소를 한 번에 물어야 하는 상황에서만 아래 압축 질문을 씁니다.
...
성공/실패 기준을 물을 때는 이 프로젝트에 기반한 예시를 함께 제시합니다. 예시는 사용자가 말한 업무 문제, 입력 자료, 결과물 형태, 사람 검토 지점, 주요 위험에서 끌어와 만들고, 일반적인 예시는 주지 않습니다.

```text
지금 프로젝트 기준으로는 `[이 프로젝트의 샘플 입력]`을 넣었을 때 `[선택한 결과물]`에 `[사람이 바로 검토할 수 있는 핵심 결과]`가 나오면 성공, `[이 프로젝트에서 특히 피해야 할 오류나 민감정보 노출]`이 남으면 실패로 볼 수 있습니다. 이 기준이 맞을까요, 아니면 현장에서 더 중요한 성공/실패 신호가 있을까요?
```
...
문제정의가 끝나면 `PLAN.md`와 `workshop.json` 초안을 만들고, 작성된 `PLAN.md` 내용을 채팅창에 보여줍니다. 그 다음 스킬은 직접 구현으로 넘어가지 않습니다. 이제 사용자가 `PLAN.md`의 3시간 구현 계획을 기준으로 구현을 시작하면 됩니다. `PLAN.md`에는 새 세션에서 구현하더라도 구현 중 결정, 변경, 막힌 점, 검증 결과를 `MEMORY.md`에 append-only로 기록하라는 구현자 지침을 포함합니다. 구현 완료 후에는 같은 폴더에서 다시 이 스킬에 발표자료를 만들어달라고 요청하면 됩니다. 그러면 산출물 범위를 묻지 않고 워크숍 표준 흐름에 따라 `WORKFLOW_ANALYSIS.md`, `CASE_STUDY.md`, 최종 `workshop.json`, 5장 Google Slides 발표자료 제작으로 이어갑니다.
